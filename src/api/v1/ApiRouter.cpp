...
void xmrig::ApiRouter::onRequest(IApiRequest& request)
{
    if (request.method() == IApiRequest::METHOD_GET) {
        ...
        else if (request.url() == "/1/diffstats") {
            request.accept();
            getDiffStats(request.reply(), request.doc());
        }
...
void xmrig::ApiRouter::getDiffStats(rapidjson::Value& reply, rapidjson::Document& doc) const
{
    using namespace rapidjson;
    auto& allocator = doc.GetAllocator();
    Value workers(kArrayType);
